...
def get_daily_steps():
    ...
    step_list = steps.split()  # ❗ faltaban los paréntesis
...
def goal_check(nums, goal=10000):
    ...
    for s in nums:
        if s >= goal:
            result.append(True)   # ❗ no string
        else:
            result.append(False)  # ❗ no string
    return result
